...
In all three examples, you can see that logistic and probit regression performed similarly. Running the models and doing the diagnostic tests was relatively easy and short in both Stata and R. However, it was inconvient in Python because Python don't have any packages for lots of tests such as the HL test. Also, the logistic regression model in sklearn packages doesn't include the function for statistics for statstical inference such as the p-values of each independent variables. It is necessary to code the tests from scratch. And the p-value computed in probit model using the Pysal package is comparatively inaccurate, which also inllustrate that Python is not so good as R and Stata while dealingwith statistics problems. This may influence your language choice if you are running similar analysis.
